...
/usr/bin/mkisofs -J -T -o $DIR/tacsim-ssg-rhel-$RHEL_VERSION.iso -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e images/efiboot.img -no-emul-boot -R -m TRANS.TBL .
cd $DIR
rm -rf $DIR/rhel-dvd
echo "Done."

echo "Signing RHEL DVD Image..."
/usr/bin/isohybrid --uefi $DIR/tacsim-ssg-rhel-$RHEL_VERSION.iso &> /dev/null
/usr/bin/implantisomd5 $DIR/tacsim-ssg-rhel-$RHEL_VERSION.iso
echo "Done."

echo "DVD Created. [tacsim-ssg-rhel-$RHEL_VERSION.iso]"
